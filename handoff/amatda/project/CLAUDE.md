...
**와이어프레임 완료. Hi-fi 디자인 준비 중.** 기획·UX 결정 완료. 미결 항목 정리 후 hi-fi 진행. 코드 없음.
...
- [x] ~~컬러 팔레트~~ — **모노 코랄** 확정. 코랄(#e8674a) 단일 색 + 명도 위계. 누르는 것=솔리드 코랄, 상태(감지중·완료·토글)=옅은 코랄 틴트(점·작게), 마커 노랑=이모지 타일. 그린은 색 충돌로 폐기(지도 공원·물 초록만 예외). Hi-fi 토큰 var(--good)/var(--green-*)를 코랄로 매핑
- [ ] 도메인 모델: "내 챙길 것" 글로벌 풀 + 담기(M:N) vs items.trigger_id(1:N) — 와이어프레임은 전자, DB 설계는 후자로 돼 있어 충돌
